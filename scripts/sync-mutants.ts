...
async function loadLocalFiles() {
  ...
  txtContent.split(/\r?\n/).forEach((line) => {
    ...
    if (parts.length >= 2) {
      ...
      // строки (баг найден в name_lore 2026-08-17, 26 записей). Отдельно (найдено
      // 2026-08-18, уникально для caption_specimen_ca_14/Брейкмастер) исходник
      // местами использует голый "/" как разделитель строк стихотворного лора
      // ("/Танцуйте", "/Докажи") - не /n, просто слэш перед заглавной буквой,
      // паттерн проверен на всём localisation_ru.txt (0 ложных срабатываний вне
      // этой записи: даты/пути/[abilities/...] не матчат, там нет пробела/\n
      // перед слэшем). Идёт ПОСЛЕ /n->\\n замены, иначе не поймает случай "\n/Таков".
      const value = parts
        .slice(1)
        .join(';')
        .trim()
