...
def sanity_check(blob: bytes) -> None:
    ...
    # fetch-filters.py берёт FB-ID прямо из блоба (User.PlatformUserId) -
    # любой аккаунт подходит без правки кода, проверять не на что.
    captured_fb = str(aj.get('User', {}).get('PlatformUserId') or '')
    if captured_fb:
        print(f'[ok] FB-ID захвата: {captured_fb}')
