...
## Аккаунт (больше не важно, какой)

`fetch-filters.py` берёт FB-ID из самого блоба (`User.PlatformUserId`), а не
хардкодит его - любой захваченный `.bin` работает без правки кода. Раньше
скрипт был жёстко привязан к одному аккаунту (`109526704649610`) и падал
`FAIL:AUTH_ERROR` на блобах с других акков - это чинили 2026-09-22.

Проверено эмпирически (2026-09-22): два разных аккаунта дали 227 из 227
идентичных дат по всем фильтрам, включая `Daily_news_tech_V2` - похоже, даты
офферов глобальные для всей игры, а не персональные сегменты. Так что для
захвата подходит любой живой аккаунт с игрой - фарм-бот, твинк, что угодно.

## Что понадобится

- Рутованный Android с MGG (любой аккаунт - см. раздел "Аккаунт" выше).
...
2. **sanity-check**: маркер `$`/`~`, xmz-decode + zlib + JSON, ключи
   `AccTok`/`DevID`/`User`;
3. печатает FB-ID захвата (`User.PlatformUserId`) - просто информационно,
   fetch-filters.py его читает из блоба и подходит любой аккаунт;
...
Любой из чеков упал → чини по сообщению (чаще всего: игра не переавторизовалась
в момент захвата - в `requests.jsonl` нет AuthService), не заливай секрет.
...
| Что | Где |
|-----|-----|
| Потребитель блоба (этот репо) | `scripts/kartel/fetch-filters.py` (FB-ID читает из блоба) |
